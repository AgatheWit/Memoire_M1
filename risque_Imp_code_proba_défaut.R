...
for (k in 1:length(p)){ # on fait une boucle for sur les individus, par tranche d'âge 
  
  actualisation_femme <- cumprod(c(1, rep(v, T-1)))
  
  p_credi_femme <- matF[(age:age+T) , cln ]  / matF[age, cln] 
  ...
  numerateur_femme[k] <- rente * p[k] *sum( actualisation_femme * p_credi_femme * sum_up )
  
  denominateur_femme[k] <- sum(p_credi_femme * (1-p[k]) * actualisation_femme)
...
for (k in 1:length(p)){ # on fait une boucle for sur les individus, par tranche d'âge 
  ...
  p_credi_homme <- matH[(age:age+T) , cln ]  / matH[age, cln] 
  ...
  numerateur_homme[k] <- rente * p[k] *sum( actualisation_homme * p_credi_homme * sum_upH )
  
  denominateur_homme[k] <- sum(p_credi_homme * (1-p[k]) * actualisation_homme)
...
plot(p, prime_annuelle_homme, type = "l", lwd = "3", col = "tomato3", main = "Prime annuelle risque rente impayées - tables TGH05/TGF05", xlab="probabilité de défaut", ylab='Prime annuelle')
lines(p, prime_annuelle_femme, type = "l", lwd = "3", col = "blue") 
legend('topleft', 
